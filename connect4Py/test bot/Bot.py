...
class c4_bot:
    ...
    def wincon1011_01010(self, player, nextpiece, horizontal, vertical):  # bot check for trap and diagonal wincon
        ...
        # if nextpiece not in oppopieces and slot1 in playerpieces and slot2 not in playerpieces + oppopieces and slot3 not in playerpieces + oppopieces:  # check for 0 1 0 1 0 wincon
        #     if int(slot2)-1 not in playerpieces + oppopieces and int(slot3)-1 not in playerpieces + oppopieces and nextpiece not in self.bot3winconlist:
        #         self.bot3winconlist.append(nextpiece)
    ...
    def nextnum(self):
        ...
        for x in pieces:
            if int(x[0]) > 3:  # checks to the left
                self.mingwincon(pieces, x, '-1', '0', self.player)

            if int(x[0]) > 3 and int(x[1]) < 4:  # checks diagonally left
                self.mingwincon(pieces, x, '-1', '1', self.player)
            ...
            if int(x[1]) < 4:  # checks upwards
                self.mingwincon(pieces, x, '0', '1', self.player)

    def mingwincon(self, pieces, nextpiece, horizontal, vertical, player):
        # selfcheck?
        ...
        missing = 0
        missinglst = []
        print(f"checking {nextpiece} for {horizontal}, {vertical}")
        # check how close the bot is to winning
        for i in range(1, 4):
            nextpiece = '%s%s' % (int(nextpiece[0]) + int(horizontal), int(nextpiece[1]) + int(vertical))
            if nextpiece in own:
                continue
            elif nextpiece in opp:
                return
            elif nextpiece not in own:
                missing += 1
                missinglst.append(nextpiece)

        if missing == 1:
            self.bot3winconlist.append(missinglst[0])
            print(f"3 list gotten is {missinglst}")
        elif missing ==2:
            self.bot2winconlist.append(missinglst)
            print(f"2 list gotten is {missinglst}")