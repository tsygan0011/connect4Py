...
class c4_bot:
    ...
    def wincon1011_01010(self, player, nextpiece, horizontal, vertical):  # bot check for trap and diagonal wincon
        ...
        # check for 0 1 0 1 0 wincon
        if slot1 in playerpieces and slot2 not in playerpieces + oppopieces and slot3 not in playerpieces + oppopieces and nextpiece not in oppopieces:  # check if its 0 1 0 1 0
            if int(slot2[1]) - 1 + int(slot3[1]) - 1 == 0 or (str(int(slot2)-1) in playerpieces + oppopieces and str(int(slot3)-1) in playerpieces + oppopieces): # check if its actually a threat
                if nextpiece not in self.bot3winconlist:  # check if its in winconlist
                    self.bot3winconlist.append(nextpiece)
