...
class c4_bot:
        ...
        for ii in self.bot2winconlist:
            for i in ii:
                ...
                if temp == 2:
                    ...
                    weights[i[0]] += 6 - (int(i[1]) - self.colheights[int(i[0])])
    ...
    def wincon1101_1100(self, player, nextpiece, horizontal, vertical):  # bot check if 2 in a row has win condition
        ...
        if nextpiece not in playerpieces + oppopieces and slot2 not in playerpieces + oppopieces and temp not in self.bot2winconlist:
            ...
            if int(vertical) == 0 and 0 < int(nextpiece[0]) < 8 and 0 < int(
                    slot2[0]) < 8 and temp2 not in self.bot2winconlist:
                self.bot2winconlist.append(temp)
    ...
    def wincon1011_01010(self, player, nextpiece, horizontal, vertical):  # bot check for trap and diagonal wincon
        ...
        if int(vertical) == 0 and slot1 in playerpieces and slot2 not in playerpieces + oppopieces and slot3 not in playerpieces + oppopieces:  # check for 0 1 0 1 0
            if nextpiece not in self.bot3winconlist:
                self.bot3winconlist.append(nextpiece)

        if slot1 in playerpieces and slot2 not in playerpieces + oppopieces and slot3 not in playerpieces + oppopieces:  # check for 0 1 0 1 0 wincon
            if int(slot2[1])-1 not in playerpieces + oppopieces and int(slot3[1])-1 not in playerpieces + oppopieces and nextpiece not in self.bot3winconlist:
                self.bot3winconlist.append(nextpiece)
