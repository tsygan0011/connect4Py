...
def build_host():
    ...
    Entry(cgCodeFr, textvariable=strv1, state=DISABLED, font=("Aerial", 52), width=2, justify="center").grid(
        row=1, column=1)
    Entry(cgCodeFr, textvariable=strv2, state=DISABLED, font=("Aerial", 52), width=2, justify="center").grid(
        row=1, column=2)
    Entry(cgCodeFr, textvariable=strv3, state=DISABLED, font=("Aerial", 52), width=2, justify="center").grid(
        row=1, column=3)
    Entry(cgCodeFr, textvariable=strv4, state=DISABLED, font=("Aerial", 52), width=2, justify="center").grid(
        row=1, column=4)
    Entry(cgCodeFr, textvariable=strv5, state=DISABLED, font=("Aerial", 52), width=2, justify="center").grid(
        row=1, column=5)
    Entry(cgCodeFr, textvariable=strv6, state=DISABLED, font=("Aerial", 52), width=2, justify="center").grid(
        row=1, column=6)

    # Text display
    cgTxtDsip = Text(cgTxtDispFr, height=16, width=104, font=("Aerial",18), bg='black', fg='white')
    ...
    startBtn.pack(side='left', padx=40)
    backBtn.pack(side='right', padx=40)

    setGameCode()
    broadcastthread = threading.Thread(target=sendGameBC, daemon=True)
    joinsniffthread =  AsyncSniffer(filter="icmp", lfilter=stopfilter, count=1, prn=establishConnection)
