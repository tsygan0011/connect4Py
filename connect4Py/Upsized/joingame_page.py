...
def build_join():
    ...
    jg_searchgame_btn_fr.pack(pady=30)
    jg_gamecode_fr = Frame(jgMainFr)
    jg_gamecode_fr.pack(pady=30)
    jg_connect_btn_fr = Frame(jgMainFr)
    jg_connect_btn_fr.pack(pady=10)
    ...
    def connect():
        ...
        if len(gc) == 6:
            try:
                ...
                send(pkt)
                startsnifferthread = AsyncSniffer(filter="icmp", lfilter=sniff4start, prn=startGamepkt)
    ...
    jg_back_btn.pack(side='right',padx=10)

    jg_page.bind("<<StartGame>>", startGame)

    gamesnifferthread = AsyncSniffer(filter="icmp", lfilter=sniff4games, prn=updateGameList)
